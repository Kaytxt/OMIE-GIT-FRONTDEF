...
function AdminDashboard({ selectedMonth, selectedYear, onGoBack, clienteInfo }) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const lancamentosResponse = await axios.get(
                `http://localhost:5000/api/admin/lancamentos?mes=${selectedMonth}&ano=${selectedYear}&clienteId=${clienteInfo.id}`,
                config
            );
    ...
    try {
        ...
        const [fornecedoresRes, categoriasRes, contasCorrentesRes] = await Promise.all([
            axios.post('http://localhost:5000/api/admin/omie/fornecedores', 
                { clienteId: clienteInfo.id }, 
                config
            ),
            axios.post('http://localhost:5000/api/admin/omie/categorias', 
                { clienteId: clienteInfo.id }, 
                config
            ),
            // NOVA CHAMADA DE API PARA O ADMIN
            axios.post('http://localhost:5000/api/admin/omie/contas-correntes', 
                { clienteId: clienteInfo.id }, 
                config
            )
        ]);
    ...
    const handleUpdateSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`http://localhost:5000/api/admin/lancamentos/${editingLancamento.id}`, 
                dataToUpdate, 
                config
            );
    ...
    const handleDeleteLancamento = async (id) => {
        if (window.confirm('Tem certeza que deseja excluir este lançamento?')) {
            try {
                ...
                await axios.delete(`http://localhost:5000/api/admin/lancamentos/${id}`, config);
    ...
    try {
        ...
        await axios.post('http://localhost:5000/api/admin/omie/lancamentos-lote', 
            {
