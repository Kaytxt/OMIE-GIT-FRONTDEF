...
function Dashboard({ selectedMonth, selectedYear, onGoBack }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [fornecedoresResponse, categoriasResponse, contasCorrentesResponse] = await Promise.all([
        axios.post('http://localhost:5000/api/omie/fornecedores'),
        axios.post('http://localhost:5000/api/omie/categorias'),
        axios.post('http://localhost:5000/api/omie/contas-correntes')
      ]);
      ...
      const lancamentosResponse = await axios.get(
        `http://localhost:5000/api/lancamentos?mes=${selectedMonth}&ano=${selectedYear}`
      );
  ...
  const handleLancar = async (e) => {
    ...
    try {
      await axios.post('http://localhost:5000/api/lancamentos', dataToSave);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/lancamentos/${editingId}`, dataToUpdate);
      await fetchData(); 
      setEditingId(null);
      setLancamento({
        fornecedor: '', contaDesmembrada: 'nao', categoria: '', contaCorrente: '',
  ...
  const handleDelete = async () => {
    if (window.confirm(`Tem certeza que deseja excluir ${selectedItems.length} lançamento(s)?`)) {
      try {
        await axios.delete('http://localhost:5000/api/lancamentos', { data: { ids: selectedItems } });
  ...
  const handleDownload = async () => {
    try {
      window.open('http://localhost:5000/api/lancamentos/download', '_blank');
